...
__host__ __device__ void sort(
  const vtkm_cuda_policy& exec, thrust::system::cuda::pointer<T> first,
...
__host__ __device__ void sort_by_key(
  const vtkm_cuda_policy& exec, thrust::system::cuda::pointer<T> first,
  thrust::system::cuda::pointer<T> last, RandomAccessIterator values_first,
  vtkm::exec::cuda::internal::WrappedBinaryPredicate<T, vtkm::SortLess> comp)
{ //sort for concrete pointers and less than op
  //this makes sure that we invoke the thrust radix sort and not merge sort
...
__host__ __device__ void sort(const vtkm_cuda_policy& exec, RandomAccessIterator first,
...
template <typename RandomAccessIteratorKeys, typename RandomAccessIteratorValues,
          typename StrictWeakOrdering>
__host__ __device__ void sort_by_key(const vtkm_cuda_policy& exec, RandomAccessIteratorKeys first,
...
template <typename T, typename InputIterator2, typename OutputIterator1, typename OutputIterator2,
...
__host__ __device__::thrust::pair<OutputIterator1, OutputIterator2> reduce_by_key(
  const vtkm_cuda_policy& exec, thrust::system::cuda::pointer<T> keys_first,
  thrust::system::cuda::pointer<T> keys_last, InputIterator2 values_first,
  OutputIterator1 keys_output, OutputIterator2 values_output, BinaryPredicate binary_pred,
...

{
  ...
  ::thrust::pair<OutputIterator1, OutputIterator2> result =
    thrust::reduce_by_key(thrust::cuda::par, keys_first.get(), keys_last.get(), values_first,
                          keys_output, values_output, binary_pred, binary_op);
...
//only sync if we are being invoked from the host
  ...
  return result;
#else
  return thrust::reduce_by_key(thrust::cuda::par, keys_first, keys_last, values_first, keys_output,
