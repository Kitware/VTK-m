...
public:
  ...
  // Compute the isosurface at the given value into the given arrays
  void computeIsosurface(const float &isovalue, vtkm::cont::ArrayHandle<vtkm::Vec<FieldType,3> > &verticesArray, vtkm::cont::ArrayHandle<FieldType> &scalarsArray)
  {
    ...
    ClassifyCell classifyCellFunctor(vertexTableArray.PrepareForInput(DeviceAdapter()), isovalue);
    vtkm::worklet::DispatcherMapTopology<ClassifyCell>(classifyCellFunctor).Invoke(
    ...
    typedef IsosurfaceSingleTri IsoSurfaceFunctor;
