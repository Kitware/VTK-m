...
//  this software.
//============================================================================
#ifndef vtk_m_worklet_Dispatcher_MapMultiBlock_h
#define vtk_m_worklet_Dispatcher_MapMultiBlock_h
...
class DispatcherMapMultiBlock :
    public vtkm::worklet::internal::DispatcherBase<
      DispatcherMapMultiBlock<WorkletType,Device>,
      WorkletType,
      vtkm::worklet::WorkletMapMultiBlock>
{
  typedef vtkm::worklet::internal::DispatcherBase<
    DispatcherMapMultiBlock<WorkletType,Device>,
    WorkletType,
    vtkm::worklet::WorkletMapMultiBlock> Superclass;

public:
  VTKM_CONT
  DispatcherMapMultiBlock(const WorkletType &worklet = WorkletType())
    : Superclass(worklet) {  }
  ...
  void DoInvoke(const Invocation &invocation) const
  {
    ...
    std::string fieldname=invocation.Parameters.template GetParameter<3>().GetPortalConstControl().Get(0);
    ...
    // array.
    //vtkm::Id numInstances = inputDomain.GetNumberOfValues()*inputDomain[0].GetNumberOfValues();
    //vtkm::Id numInstances = inputDomain.GetNumberOfValues()*inputDomain.GetPortalControl().Get(0).GetNumberOfCellSets();
    
    vtkm::Id numInstances = inputDomain.GetNumberOfValues()*inputDomain.GetPortalConstControl().Get(0).GetField(fieldname).GetData().GetNumberOfValues();
    std::cout<<numInstances<<"name";
